...
export function Sidebar({ user }: { user: NavUser | null }) {
  ...
  const main: NavItem[] = [
    { href: "/", label: "Projekte", icon: <IconGrid />, match: (p) => p === "/" || p.startsWith("/projekte") },
  ];
  if (user?.canUpload) main.push({ href: "/upload", label: "Upload", icon: <IconUpload />, match: (p) => p.startsWith("/upload") });
  if (user?.canBrand) main.push({ href: "/marke", label: "Markenprofil", icon: <IconBrand />, match: (p) => p.startsWith("/marke") });
  ...
  if (canExt(user?.role, "experiments.manage")) publishing.push({ href: "/experimente", label: "Experimente", icon: <IconFlask />, match: (p) => p.startsWith("/experimente") });
  if (user) publishing.push({ href: "/berichte", label: "Berichte", icon: <IconChart />, match: (p) => p.startsWith("/berichte") });

  const tools: NavItem[] = [];
  if (can(user?.role, "api.manage")) tools.push({ href: "/entwickler", label: "Entwickler", icon: <IconCode />, match: (p) => p.startsWith("/entwickler") });

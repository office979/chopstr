    ...
    return withContext(await currentSession(), async (tx) => {
      const rows = await tx`
        select c.id, c.source_id, c.status, c.review,
               ...
               c.effekte, c.render_plan->'effekte' as plan_effekte,
      ...
      return (rows as Row[]).map((r) => ({
        ...
        effekte: jsonValue<ClipStand["effekte"]>(r.effekte, null),
        plan_effekte: jsonValue<ClipStand["plan_effekte"]>(r.plan_effekte, null),
